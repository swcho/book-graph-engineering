...
같은 페이지의 영어판은 [`content_en/`](content_en)에 있습니다. 예제 코드는 언어별로 복사하지 않았습니다. 코드는 한 벌이고 `content/`에 있습니다.
...
본문에 **쓰지 않은** 자료는 [바깥 읽을거리](LINKS.md)에 따로 모았습니다. 논문, 교과서, 강의, 엔진, 공개 데이터, 그리고 빅테크가 같은 문제를 어떻게 풀었는지까지 106개를 표로만 정리했고 관련 장을 붙여 뒀습니다. 책을 읽다가 더 파고 싶어졌을 때 다음으로 열 것들입니다.
...
| **자료 제안** | [바깥 읽을거리](LINKS.md)에 넣을 자료를 찾았을 때. 죽은 링크 신고도 | [작성](../../issues/new?template=04-link-suggestion.yml) |
